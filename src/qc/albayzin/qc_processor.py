...
class AlbayzinQCProcessor:
    """QC processor for ALBAYZIN dataset."""
    ...
    def __init__(
        ...
        Args:
            index_path: Path to utt_index_albayzin.csv from Block B
    ...
    def process_all(
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), desc="QC ALBAYZIN"):
            try:
    ...
    def process_utterance(
        ...
        try:
            ...
            # Calculate frame energies (used for silence and SNR)
    ...
    def apply_thresholds(self, metrics: QCMetrics) -> QCMetrics:
        ...
        if np.isfinite(metrics.snr_db):
            ...
            # No speech detected - fail SNR
            failed_checks.append('snr')
        # Note: +inf SNR (no noise) is considered passing
