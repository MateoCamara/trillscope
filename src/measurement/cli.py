...
def measure_dataset(
    ...
    if not candidates_path.exists():
        ...
        logger.warning(f"Run Block F extraction first: python -m src.extraction.cli extract {dataset}")
        return None
...
Examples:
  ...
  python -m src.measurement.cli measure all

  # Measure specific dataset
  python -m src.measurement.cli measure dimex100

  # Include tokens without timing (PRESEEA orthographic)
  python -m src.measurement.cli measure all --include-invalid-timing

  # Show default configuration
  python -m src.measurement.cli show-config
