...
def normalize_dataset(dataset: str, metadata_dir: Path,
    ...
    if not parquet_path.exists():
        ...
        logger.warning(f"Run Block A ingestion first: python -m src.ingestion.cli ingest {dataset}")
        return None
...
Examples:
  ...
  python -m src.metadata.cli normalize albayzin

  # Normalize PRESEEA metadata
  python -m src.metadata.cli normalize preseea

  # Normalize all and merge into unified parquet
  python -m src.metadata.cli normalize all

  # Preview education mapping rules
  python -m src.metadata.cli preview albayzin
