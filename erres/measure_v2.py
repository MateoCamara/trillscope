"""Bridge from v1 token tables to detector v2.

Reads r_candidates_<dataset>.parquet (produced by src/extraction), loads the
original audio referenced by each row, runs detector v2 on a window padded by
DetectorConfig.boundary_expand_ms around the token, and writes
closures_v2_<dataset>.parquet.

This is the temporary glue that lets us evaluate v2 on existing tokens before
the corpora/alignment/extraction modules of erres are written.
"""
