...
path uses in ``erres.cli``.

Run:
    python -m erres.detect_unfiltered
"""
...
def token_keys_by_corpus(tokens_path: Path) -> dict[str, set]:
    ...
    Mirrors erres.cli._load_filter_keys but applies NO quality filter, so every
...
def main() -> None:
    ...
    cfg = load_config(Path("config/detector.yaml"))
